...
class PLMSSamplerInst(object):
 ...
 def forward(self, instance_features, boxes, ...):
        """
        instance_features: List[Tensor] or Tensor，表示每个实例的特征 (B, C, H, W)
        boxes: List[Tuple] 或 Tensor，表示每个实例的边界框 [x_min, y_min, x_max, y_max]
        """

       
        fused = self.fuse_instance_features(instance_features, ...)

        occlusion_mask = torch.ones_like(fused)
        num_inst = len(boxes)
        for i in range(num_inst):
            for j in range(num_inst):
                if i == j:
                    continue
                if self.is_occluded(boxes[i], boxes[j]):
                  
                    mask = self.compute_overlap_mask(boxes[i], boxes[j], fused.shape[-2:])
                    occlusion_mask[i] = occlusion_mask[i] * (1.0 - mask)
      
        fused = fused * occlusion_mask

        return fused

    def is_occluded(self, box1, box2):
        ...
        overlap = not (x1_max < x2_min or x1_min > x2_max or
                       y1_max < y2_min or y1_min > y2_max)
        return overlap and (area1 < area2)  # 更小的遮挡更大的

    def compute_overlap_mask(self, box1, box2, spatial_shape):
        
        H, W = spatial_shape
        mask = torch.zeros((H, W), device='cuda')
        x1_min, y1_min, x1_max, y1_max = [int(v*W) if idx%2==0 else int(v*H) for idx,v in enumerate(box1)]
        x2_min, y2_min, x2_max, y2_max = [int(v*W) if idx%2==0 else int(v*H) for idx,v in enumerate(box2)]
        ...
        return mask.unsqueeze(0).unsqueeze(0)  # [1,1,H,W] 便于广播
